...
class qaoa_qutip(object):
    ...
    def edge_noise(self, q):
        ''' Applies noise to edge ZZ gate. Following Wang et al. Nature, a ZZ 
        gate is decomposed into  CNOT RZ[2] CNOT where RZ is the parametric
        gate acting on the second qubit and is the only parametric gate, 
        so the only on experiencing noise'''
        
        noise = self.Id
        
        for edge in self.G.edges:
            noise *= self.Rx(edge[1], q)
        
        return noise
    
    def mixing_noise(self, q):
        ''' Applies noise to edge X gates. Following Wang et al. Nature, we 
        apply the error before and after each parametric X gate'''
...
#         exit()
    ...
    def layer(self, state_0, gamma, beta):
        state_0 = self.U_c(gamma) * state_0
        self.states.append(state_0)
        state_0 = self.U_mix(beta) * state_0
        self.states.append(state_0)
        
        return state_0
        
    def noise_layer(self, state_0, gamma, beta):
        '''
        Noise added following Wang et al. (Nature) convention of applying a
        q_z * Z error to each single qubit parametric Z gate and so on'''
        
        q_x, q_y, q_z = np.random.uniform(-1, 
                                          1, 
                                          3)
        print(q_x, q_y, q_z)
        state_0 = self.edge_noise(q_z) * state_0
        state_0 = self.U_c(gamma) * state_0
        state_0 = self.edge_noise(-q_z) * state_0
        self.states.append(state_0)
        
        state_0 = self.mixing_noise(q_x) * state_0
        state_0 = self.U_mix(beta) * state_0
        state_0 = self.mixing_noise(-q_x) * state_0
        self.states.append(state_0)
    ...
    def quantum_algorithm(self,
        ...
        for p in range(depth):
            ...
            if self.gate_noise is None:
                print('noiseless')
                state_0 = self.layer(state_0, 
                                     gammas[p], 
                                     betas[p])
            else:
                print('noisy')
                state_0 = self.noise_layer(state_0, 
                                           gammas[p], 
                                           betas[p])
            #self.states.append(state_0)
