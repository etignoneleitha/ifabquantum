...
RUNS A LOOP ON BOND DISTANCES AND OBTAINS THE BEST OPTIMIZED ENERGY
WITH BAYES AT A FIXED P to reconstruct the full diagram of the bond energy
...
nbayes = args.nbayes
how_many_bonds = args.how_many_bonds
...
bond_distances = np.linspace(0.01, 2, how_many_bonds)
...
for bond_distance in bond_distances:
    ...
    qaoa = qaoa_qutip(G, problem=problem, bond_distance = bond_distance)
    ...
    for i in range(nbayes):
        ...
        approx_ratio = mean_energy/qaoa.gs_en
        if depth <2:
            acq_func = gp.get_acquisition_function(show = False, save = False)
